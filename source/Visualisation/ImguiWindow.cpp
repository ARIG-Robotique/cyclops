...
ImguiWindow::ImguiWindow(string InWindowName)
...
void ImguiWindow::AddImageToBackground(const Texture &Image, cv::Rect impos, cv::Size2f UVmin, cv::Size2f UVmax)
{
	...
	ImDrawList* background = ImGui::GetBackgroundDrawList();
	{
		...
		//cout << "Adding " << Image.GetTextureID() << " to background" << endl;
		background->AddImage((ImTextureID)(intptr_t)Image.GetTextureID(), p_min, p_max, ImVec2(UVmin), ImVec2(UVmax));
	}
}

void ImguiWindow::AddImageToBackground(unsigned int slot, const cv::UMat &Image, cv::Rect impos, cv::Size2f UVmin, cv::Size2f UVmax)
{
	if (slot >= Textures.size())
	{
		Textures.resize(slot+1);
	}
	auto& tex = Textures[slot];
	tex.LoadFromUMat(Image);
	AddImageToBackground(tex, impos, UVmin, UVmax);
}